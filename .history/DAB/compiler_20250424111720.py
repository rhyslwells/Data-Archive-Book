...
CODE_FENCE = re.compile(r'([^\n])```')  # Ensure a blank line before the code fence if it's not already on a line
...
for md in sorted(CONTENT_DIR.glob("*.md"), key=lambda p: p.name.lower()):
    ...
    # Fix code fence: Ensure that there is a blank line before the code block
...
print("✓ Preface added, code‑fence fix applied, book/book.md and SUMMARY.md regenerated.")
