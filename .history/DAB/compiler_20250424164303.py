...
MATH_BLOCK = re.compile(r'^[ \t]*\$\$[ \t]*\n(.*?)[ \t]*\n[ \t]*\$\$[ \t]*$',
                        re.S | re.M)
ONE_LINE_MATH = re.compile(r'^[ \t]*\$\$(.+?)\$\$[ \t]*$', re.M)

def slugify(t: str) -> str:
    return re.sub(r'[^\w\- ]', '', t).strip().lower().replace(' ', '-')

def protect_math(text: str) -> str:
    """Wrap any $$ … $$ in raw/endraw so Nunjucks ignores it."""
    # 1‑line → multi‑line then wrap
    text = ONE_LINE_MATH.sub(lambda m:
        '{% raw %}\n$$\n' + m.group(1).strip() + '\n$$\n{% endraw %}', text)
    text = MATH_BLOCK.sub(lambda m:
        '{% raw %}\n$$\n' + m.group(1).strip() + '\n$$\n{% endraw %}', text)
    return text
...
for md in sorted(CONTENT_DIR.glob("*.md"), key=lambda p: p.name.lower()):
    ...
    text = protect_math(text)                          # <── math fix
...
print("✓ Preface added, maths wrapped safely, book/book.md & SUMMARY.md regenerated.")
