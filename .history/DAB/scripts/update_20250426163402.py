...
def update_links_and_titles(file_path):
    ...
    def replace_image(match):
        ...
        image_name = raw_content.split("|")[0].strip()  # Take only the part before '|'
        encoded = image_name.replace(' ', '%20')
        return f'![{image_name}](../content/images/{encoded})'
...
for root, _, files in os.walk(str(content_folder)):
    for file in files:
        if file.endswith('.md'):
