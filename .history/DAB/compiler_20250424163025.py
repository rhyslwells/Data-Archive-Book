...
MATH_BLOCK = re.compile(r'^[ \t]*\$\$[ \t]*\n(.*?)[ \t]*\n[ \t]*\$\$[ \t]*$',
                        re.S | re.M)
# single‑line $$  …  $$ block
MATH_ONE   = re.compile(r'^[ \t]*\$\$(.+?)\$\$[ \t]*$', re.M)

def slugify(t: str) -> str:
    return re.sub(r'[^\w\- ]', '', t).strip().lower().replace(' ', '-')

def protect_math(t: str) -> str:
    """Wrap both multi‑line and one‑line $$…$$ fences in raw/endraw."""
    # convert one‑liners to multi‑line then wrap all blocks
    t = MATH_ONE.sub(lambda m:
        '{% raw %}\n$$\n' + m.group(1).strip() + '\n$$\n{% endraw %}', t)
    t = MATH_BLOCK.sub(lambda m:
        '{% raw %}\n$$\n' + m.group(1).strip() + '\n$$\n{% endraw %}', t)
    return t
...
print("✓ Preface added, math fences secured, book/book.md and SUMMARY.md regenerated.")
